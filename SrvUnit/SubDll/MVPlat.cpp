...
int  MVPlatIO::RestoreDataFrame(const char * lpInBuf,unsigned short sInSize,char * * lpOutPtr,char * lpTempBuf,unsigned short sTempSize )
{
	...
	{
		//解析数据失败
		Global_WriteLog(3,0,"MVPlatIO","解析接收到的数据发生错误1");
		return(errorcode);
	}
	else if ( errorcode > COMM_MAX_FRAME_SIZE )
	{
		//解析数据长度失败
		Global_WriteLog(3,0,"MVPlatIO","解析接收到的数据长度错误2");
		return(-1);
	}
	...
	{
		Global_WriteLog(3,0,"MVPlatIO","解析接收到的数据发生错误[本平台不支持数据特殊处理]3");
		return(-2);
	}
	...
	{
		Global_WriteLog(3,0,"MVPlatIO","解析接收到的数据发生错误4");
		return(-3);
	}
