...
int		MSHL2Dll::outer_GetLastMarketDataAll(char* pszInBuf, int32_t nInBytes)
{
...
int		MSHL2Dll::inner_getLastStockData(int nStockCount, MStreamWrite& oMSW)
{
...
int		MSHL2Dll::inner_getLastIndexData(int nIndexCount, MStreamWrite& oMSW)
{
	unsigned int MsgType =0;
...
bool	MSHL2Dll::outer_ExpectData(unsigned char MainType, unsigned char ChildType, unsigned char MarketID)
{
	if (MarketID == XDF_SHL2 && MainType ==2)
	{
		if (ChildType == 42)
		{
			return true;
...
//[2, 42]	//0 -市场信息  2 -指数 25 -个股
void	MSHL2Dll::outer_OnPushData(unsigned char ChildType, const char* pszbuf, unsigned short bufsize)
{
	//[1]//更新市场时间,更新iRecv
	m_i64Recv += bufsize;
	...
	if (ChildType == 42)
	{
		...
		while(nLen >0)
		{
			...
			case 8:		//盘前虚拟价格 忽略
				{
					tagCcComm_VirtualPrice* pData = (tagCcComm_VirtualPrice*)pbuf;
					
					pbuf += sizeof(tagCcComm_VirtualPrice);
					nLen -= sizeof(tagCcComm_VirtualPrice);
				}
				break;
			case 3:		//逐笔成交 忽略
				{
					pbuf += sizeof(tagCcComm_ShL2TransData);
					nLen -= sizeof(tagCcComm_ShL2TransData);
				}
				break;
			case 7:		//委托队列 忽略
				{
					int nemit = sizeof(tagCcComm_OrderQueueHeadL2Fast)+sizeof(tagCcComm_OrderPrice);
					int ndetail = sizeof(tagCcComm_OrderVolDetail);
					
					tagCcComm_OrderPrice* pPrice = (tagCcComm_OrderPrice*)(pbuf + sizeof(tagCcComm_OrderQueueHeadL2Fast));
...
void	MSHL2Dll::inner_GetIndexInfo(unsigned int& MsgType, int& sizeComIndexData, int& sizeXDFIndexData)
{
	MsgType = 21;
...
void	MSHL2Dll::inner_GetStockInfo(unsigned int& MsgType, int& sizeComStockData, int& sizeXDFStockData)
{
	MsgType = 30;
...
void	MSHL2Dll::inner_GetNameTableInfo(unsigned int& MsgType, int& sizeComNameTable, int& sizeXDFNameTable)
{
	MsgType = 5;
...
//[2, 2]码表
int		MSHL2Dll::inner_GetNameTable(unsigned short dwStartNo, void * pValue, int nCount)
{
...
//[2, 13]个股
int		MSHL2Dll::inner_GetStock(unsigned short dwStartNo, void * pValue, int nCount)
{
	assert( pValue != NULL && nCount > 0 );
...
//[2, 6]指数
int		MSHL2Dll::inner_GetIndex(unsigned short dwStartNo, void * pValue, int nCount)
{
	assert( pValue != NULL && nCount > 0 );
...
int		MSHL2Dll::inner_TransIndexData(void* pData, char* poutbuf)
{
...
int		MSHL2Dll::inner_TransNameTable( void * pData, char* poutbuf)
{
...
int		MSHL2Dll::inner_TransStcok(void* pValue, char* poutbuf)
{
...
bool	MSHL2Dll::inner_CompareStock(void* pData, char* pCode)
{
...
bool	MSHL2Dll::inner_IndexCompare(void* pData, char* pCode)
{
...
int		MSHL2Dll::inner_TransMarketStatus(void* pData, char* poutbuf)
{
